...
void swmm_nodeFlood(char* path)
{
    ...
    double t;
    FILE* noderpt = NULL;
    noderpt = fopen(path, "w");
    if (!noderpt)
    {
        return;
    }
    if (RouteModel != DW) {
        fprintf(noderpt, "%s", "**********Error:Not using DW option**********");
        fclose(noderpt);
        return;
    }
    fprintf(noderpt, "%s,%s,%s,%s,%s,%s", "Node Name", "Flooding Hours", "Maximum Rate/m^3*s^-1", "Day of Max Occurrence", "Time of Max Occurrence", "Total Flood Volume/m^3");
    for (j = 0; j < Nobjects[NODE]; j++)
    {
        if (Node[j].type == OUTFALL) continue;
        t = NodeStats[j].timeFlooded;
        if (t > 0.0) {
            t = MAX(0.01, (t / 3600.0));
        }

        fprintf(noderpt, "\n%s", Node[j].ID);                            //column1
        fprintf(noderpt, ",%.2f", t);                                   //column2
        fprintf(noderpt, ",%.4f", NodeStats[j].maxOverflow * UCF(FLOW)); //column3
        getElapsedTime(NodeStats[j].maxOverflowDate, &days, &hrs, &mins);
        fprintf(noderpt, ",%d", days);                                  //column4
        fprintf(noderpt, ",%02d:%02d", hrs, mins);					   //column5
        fprintf(noderpt, ",%.4f", NodeStats[j].volFlooded * UCF(VOLUME));  //column6   Á¢·½Ã×
    }
    fclose(noderpt);
